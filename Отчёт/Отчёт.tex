...
\begin{document}
	...
	\centering{
		...
		Выбрать параметры аналогового регулятора портала станка с числовым
программным управлением. Угол поворота вала двигателя $\varphi_p$ и перемещение
портала $x$ измеряются. Пропорциональные им сигналы $q_p = k \varphi_p / i$ и $q_m =
k x$ подаются на блок сравнения с задающим напряжением $q^* = k x^*$,
где $k = 200$ $\frac{\text{В}}{\text{м}}$ — коэффициент преобразования измерителя пройденного
пути;
$i = 250$ $\frac{\text{рад}}{\text{м}}$ — отношение угловой скорости вала двигателя к скорости портала;
$x^* = 1$ м — требуемое перемещение портала.

Сигнал рассогласования $k \psi_m = q_m - q^*$ поступает на ПИ-регулятор.
Сформированный отрицательной обратной связью сигнал управления
поступает на вход двигателя.
