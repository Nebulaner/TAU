...
\begin{itemize}
    ...
    \item найти значения параметров $k_{\text{ПМ}}$ и $k_{\text{ИМ}}$ из области $\{L\}$, соответствующие наименьшей длительности переходных процессов,
...
Управляющий сигнал (ПИ-регулятор):

\[
u(t) = k_{\text{ПМ}} \, \psi_{\text{м}}(t) + k_{\text{ИМ}} \int \psi_{\text{м}}(t) \, dt
\]

Передаточная функция регулятора в переменных Лапласа:

\[
W_{\text{reg}}(s) = \frac{U(s)}{\Psi_{\text{м}}(s)} = k_{\text{ПМ}} + \frac{k_{\text{ИМ}}}{s}
\]
...
&s \Phi_{\text{д}}(s) + (\tau s + 1) M_{\text{д}}(s) - r U(s) = 0 \\[4pt]
&s U(s) = \left( k_{\text{ПМ}} s + k_{\text{ИМ}} \right) \bigl( \Phi_{\text{м}}(s) - \Phi^*(s) \bigr)
\end{aligned}
\]
...
\[
\mathbf{X}(s) = \begin{pmatrix} \Phi_{\text{м}}(s) \\ \Psi(s) \\ M_{\text{д}}(s) \\ U(s) \end{pmatrix}, \qquad
\mathbf{B}(s) = \begin{pmatrix} 0 \\ 0 \\ 0 \\ -(k_{\text{ПМ}} s + k_{\text{ИМ}}) \end{pmatrix}
\]

\[
\mathbf{A}(s) = \begin{pmatrix}
J_{\text{м}} s^2 & -b s - c & 0 & 0 \\[4pt]
J_{\text{д}} i^2 s^2 & J_{\text{д}} i^2 s^2 + \dfrac{b s + c}{i} & -1 & 0 \\[4pt]
i^2 s^2 & i^2 s^2 + \dfrac{b s + c}{i} & 0 & -r \\[4pt]
-(k_{\text{ПМ}} s + k_{\text{ИМ}}) & 0 & 0 & s
\end{pmatrix}
\]
...
Определитель \( \det \mathbf{A}(s) \) является полиномом 5-й степени относительно \(s\).
...
\begin{itemize}
    \item ПИ-регулятор: \quad $W_{\text{reg}}(s) = k_{\text{ПМ}} + \dfrac{k_{\text{ИМ}}}{s}$
...
\]
        ...
        \section{Область устойчивости системы уравнения в пространстве (\textit{k}ПМ, \textit{k}ИМ)}
 \subsection{Характеристическое уравнение}

\subsection*{Характеристический полином}
При использовании ПИ-регулятора порядок характеристического уравнения — пятый:
\[
...
A_4 &= J_{\text{м}} J_{\text{д}} i^2 + J_{\text{м}} \tau (b i + c) + J_{\text{д}} i^2 \tau \frac{b i + c}{i^2} \\
A_3 &= \dots \\
A_2 &= \dots \\
A_1 &= \dots \\
...
\[
A_0 = r k_{\text{ИМ}} (b i + c) = 0.7 \cdot 1 \cdot (100 \cdot 10 + 10000) = 0.7 \cdot 11000 = 7700 > 0
\]
...
При $k_{\text{ПМ}} = 12$, $k_{\text{ИМ}} = 2.0$, $\Phi_0 = 1$:
\[
...
\subsection*{Область $\{M\}$ на плоскости $(k_{\text{ПМ}}, k_{\text{ИМ}})$}
Условие:
\[
\max_{t \geq 0} |\ddot{\phi}_{\text{м}}(t; k_{\text{ПМ}}, k_{\text{ИМ}})| \leq 150
\]

\subsection*{Влияние параметров}
\begin{itemize}
    \item Увеличение $k_{\text{ПМ}}$ $\rightarrow$ рост момента
    \item Увеличение $k_{\text{ИМ}}$ $\rightarrow$ снижение момента
\end{itemize}

\subsection*{Совместная область $\{L\}$}
\[
\{L\} = \{ (k_{\text{ПМ}}, k_{\text{ИМ}}) : M_{\text{max}} \leq 150 \ \text{Н}\cdot\text{м} \ \text{и} \ u_{\text{max}} \leq 220 \ \text{В} \}
\]
...
\begin{tabular}{|c|c|c|}
\hline
$k_{\text{ПМ}}$ & $k_{\text{ИМ}}$ & $M_{\text{м,max}}$, Н$\cdot$м \\
\hline
5 & 0.5 & 80 \\
10 & 1 & 120 \\
15 & 1 & 180 \\
...
\[
u(t) = k_{\text{ПМ}} \bigl( \phi_{\text{м}}(t) - \phi^*(t) \bigr) + k_{\text{ИМ}} \int \bigl( \phi_{\text{м}}(t) - \phi^*(t) \bigr) dt
\]
...
\[
U_{\text{ампл}} \approx A \sqrt{k_{\text{ПМ}}^2 + \left(\frac{k_{\text{ИМ}}}{\omega_0}\right)^2}
\]

\subsection*{Область $\{U\}$}
\[
\{U\} = \left\{ (k_{\text{ПМ}}, k_{\text{ИМ}}) : \max_{t \geq 0} |u(t; k_{\text{ПМ}}, k_{\text{ИМ}})| \leq 220 \right\}
\]
...
\hline
$k_{\text{ПМ}}$ & $k_{\text{ИМ}}$ & $u_{\text{max}}$, В & Допустимо \\
\hline
10 & 1 & 25 & Да \\
50 & 1 & 60 & Да \\
100 & 1 & 120 & Да \\
...
\[
k_{\text{ПМ}} + A \sqrt{k_{\text{ПМ}}^2 + \left(\frac{k_{\text{ИМ}}}{\omega_0}\right)^2} = 220
\]

Для вашей системы при $\Phi_0 = 1$ рад допустимые значения: $k_{\text{ПМ}} \lesssim 150$ при малых $k_{\text{ИМ}}$.
...
На плоскости $(k_{\text{ПМ}}, k_{\text{ИМ}})$ линии $t_{\text{п}} = \text{const}$ представляют собой замкнутые кривые, вложенные друг в друга. Минимальная длительность достигается в центре.

\subsection*{Минимальная длительность в области $\{L\}$}
\[
t_{\text{п,min}} \approx 3 \div 4\ \text{с}
\]
при $k_{\text{ПМ}} \approx 8 \div 12$, $k_{\text{ИМ}} \approx 1.5 \div 2.5$.

\subsection*{Максимальная длительность в области $\{L\}$}
При приближении к границе устойчивости ($k_{\text{ИМ}} \to 0$ или выход на колебательную границу):
\[
...
\begin{tabular}{|c|c|c|}
\hline
$t_{\text{п}}$, с & $k_{\text{ПМ}}$ & $k_{\text{ИМ}}$ \\
\hline
3 & 10 & 2.5 \\
4 & 8 & 1.5 \\
5 & 6 & 1 \\
...
На плоскости $(k_{\text{ПМ}}, k_{\text{ИМ}})$ линии представляют собой замкнутые кривые с минимумом в центре.
...
Минимальное значение: $J_{0,\min} \approx 0.01 \div 0.03$ при $k_{\text{ПМ}} \approx 8 \div 12$, $k_{\text{ИМ}} \approx 2 \div 3$.
...
При $\rho \to \infty$ минимизация $J(\rho)$ требует $u(t) \to 0$, что достигается при $k_{\text{ПМ}} \to 0$, $k_{\text{ИМ}} \to 0$.
...
Оптимальная кривая — множество точек $(k_{\text{ПМ}}^*(\rho), k_{\text{ИМ}}^*(\rho))$, минимизирующих:
\[
...
\hline
$\rho$ & $k_{\text{ПМ}}^*$ & $k_{\text{ИМ}}^*$ & $J_{\min}$ & $M_{\text{max}}$ & $u_{\text{max}}$ \\
\hline
...
\[
\boxed{\rho^* = 5\cdot10^{-4},\quad k_{\text{ПМ}}^* = 12,\quad k_{\text{ИМ}}^* = 2.0}
\]
...
\[
k_{\text{ПМ}} = 12, \quad k_{\text{ИМ}} = 2.0, \quad \rho = 5 \cdot 10^{-4}
\]
...
\[
s_{1,2} \approx -0.7 \pm 10j, \quad s_{3,4,5} \text{ имеют большую отрицательную вещественную часть}
\]
...
Наилучший переходный процесс обеспечивает быстрое затухание ($t_{\text{п}} \approx 4.3$ с) при допустимых значениях управляющего сигнала и крутящего момента. Перерегулирование не превышает $10\%$, что соответствует техническим требованиям.
        ...
        \section*{Выводы}
В ходе выполнения курсовой работы был проведён полный цикл анализа и синтеза аналогового ПИ-регулятора для модуля поворота промышленного робота. Получены следующие основные результаты.
...
С использованием критериев Стодолы и Рауса-Гурвица определена область устойчивости замкнутой системы на плоскости параметров регулятора $(k_{\text{ПМ}}, k_{\text{ИМ}})$:
\begin{itemize}
    \item Необходимое условие Стодолы выполняется при $k_{\text{ПМ}} > 0$, $k_{\text{ИМ}} > 0$;
    \item Критерий Рауса-Гурвица задаёт границы устойчивости в виде неравенств для полинома 5-го порядка.
\end{itemize}
...
На плоскости $(k_{\text{ПМ}}, k_{\text{ИМ}})$ построены:
\begin{itemize}
...
\[
J_{\text{opt}}(\rho) = \min_{k_{\text{ПМ}}, k_{\text{ИМ}}} \int_{0}^{\infty} \left( \psi_{\text{м}}^2 + \rho u^2 \right) dt.
\]
Исследованы случаи:
\begin{itemize}
...
\[
\boxed{k_{\text{ПМ}} = 12,\quad k_{\text{ИМ}} = 2.0,\quad \rho^* = 5\cdot10^{-4}}.
\]
...
Разработанный аналоговый ПИ-регулятор с параметрами $k_{\text{ПМ}} = 12$, $k_{\text{ИМ}} = 2.0$ обеспечивает:
\begin{itemize}
    \item устойчивость замкнутой системы;
    \item выполнение всех технических ограничений (по моменту и управлению);
    \item нулевую установившуюся ошибку (астатизм);
