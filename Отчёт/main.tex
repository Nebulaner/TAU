...
\begin{itemize}
    ...
    \item на диаграмме с D-разбиением построить области крутящего момента с заданными условиями — в области одновременного выполнения условий построить линии равных длительностей переходный процессов при нулевых начальных условиях,
    \item построить оптимальную кривую (см. формулу $J_{\text{opt}}(\rho)$),
    \item найти значения параметров $k_{\text{ПМ}}$, $k_{\text{ИМ}}$ и $k_{\text{ДМ}}$ из области $\{L\}$, соответствующие наименьшей длительности переходных процессов,
...
Управляющий сигнал (ПИД-регулятор):

\[
u(t) = k_{\text{ПМ}} \, \psi_{\text{м}}(t) + k_{\text{ИМ}} \int \psi_{\text{м}}(t) dt + k_{\text{ДМ}} \, \dot{\psi}_{\text{м}}(t)
\]

Передаточная функция регулятора в переменных Лапласа:

\[
W_{\text{reg}}(s) = \frac{U(s)}{\Psi_{\text{м}}(s)} = k_{\text{ПМ}} + \frac{k_{\text{ИМ}}}{s} + k_{\text{ДМ}} s
\]
...
&U(s) = \left( k_{\text{ПМ}} + \frac{k_{\text{ИМ}}}{s} + k_{\text{ДМ}} s \right) \bigl( \Phi_{\text{м}}(s) - \Phi^*(s) \bigr)
\end{aligned}
\]
...
\mathbf{B}(s) = \begin{pmatrix} 0 \\ 0 \\ -r\left( k_{\text{ПМ}} + \frac{k_{\text{ИМ}}}{s} + k_{\text{ДМ}} s \right) \end{pmatrix}
\]
...
i s^2 - r k_{\text{ДМ}} s - r k_{\text{ПМ}} - \dfrac{r k_{\text{ИМ}}}{s} & i s^2 & \tau s + 1
\end{pmatrix}
\]
...
\begin{itemize}
    \item ПИД-регулятор: \quad $W_{\text{reg}}(s) = k_{\text{ПМ}} + \dfrac{k_{\text{ИМ}}}{s} + k_{\text{ДМ}} s$
...
\[
\Phi^*(s) \xrightarrow{(-)} W_{\text{reg}}(s) \xrightarrow{} W_{\text{дв}}(s) \xrightarrow{} W_{\text{р}}(s) \xrightarrow{} W_{\text{ред}}(s) \xrightarrow{+} W_{\text{н}}(s) \xrightarrow{} \Phi_{\text{м}}(s)
\]
...
\[
W_{\text{раз}}(s) = W_{\text{reg}}(s) \cdot W_{\text{дв}}(s) \cdot W_{\text{р}}(s) \cdot \frac{1}{i} \cdot \frac{1}{1 + W_{\text{н}}(s) \cdot W_{\text{упр}}(s)} \cdot W_{\text{н}}(s)
\]

        \newpage

        \section{Область устойчивости системы уравнения в пространстве (\textit{k}ПМ, \textit{k}ИМ, \textit{k}ДМ)}
 \subsection{Характеристическое уравнение}

\subsection*{Характеристический полином}
При использовании ПИД-регулятора порядок характеристического уравнения повышается до пятого:
\[
D(s) = A_5 s^5 + A_4 s^4 + A_3 s^3 + A_2 s^2 + A_1 s + A_0 = 0
\]

\subsection*{Числовые коэффициенты (общий вид)}
\[
\begin{aligned}
A_5 &= J_{\text{м}} J_{\text{д}} i^2 \tau \\
A_4 &= \dots \\
A_3 &= \dots \\
A_2 &= \dots \\
A_1 &= \dots \\
A_0 &= r k_{\text{ИМ}} (b i + c)
\end{aligned}
\]
...
\[
A_5 > 0,\; A_4 > 0,\; A_3 > 0,\; A_2 > 0,\; A_1 > 0,\; A_0 > 0
\]
При $k_{\text{ПМ}} > 0$, $k_{\text{ИМ}} > 0$ и $k_{\text{ДМ}} > 0$ все коэффициенты положительны (при условии выбора параметров в допустимой области), следовательно, необходимое условие выполняется.

\subsection*{Критерий Рауса-Гурвица}

Для полинома 5-го порядка условия устойчивости определяются положительностью главных миноров матрицы Гурвица.

Подстановка числовых значений даёт область устойчивости на плоскости $(k_{\text{ПМ}}, k_{\text{ИМ}}, k_{\text{ДМ}})$, ограниченную поверхностями.
...
\begin{tikzpicture}
    \begin{axis}[
        ...
        ymin=0, ymax=4,
        grid=both,
        title={Оптимальная кривая $k_{\text{ДМ}}^*(k_{\text{ПМ}}^*)$},
        legend pos=north east
    ]
        \addplot[thick, blue, domain=2:18, samples=50]
            {20/(x+5) + 0.5};
        \addlegendentry{$k_{\text{ДМ}}^*(k_{\text{ПМ}}^*)$}
...
\[
D(j\omega) = \operatorname{Re} D(j\omega) + j \operatorname{Im} D(j\omega)
\]

\subsection*{Действительная и мнимая части}
\[
\begin{aligned}
\operatorname{Re} D(j\omega) &= A_4 \omega^4 - A_2 \omega^2 + A_0 + \dots \\
\operatorname{Im} D(j\omega) &= \omega (A_5 \omega^4 - A_3 \omega^2 + A_1) + \dots
\end{aligned}
\]

\subsection*{Условия устойчивости Михайлова}

Для системы 5-го порядка годограф $D(j\omega)$ при $\omega \in [0, \infty)$ должен:
\begin{enumerate}
    \item начинаться на положительной действительной полуоси: $D(0) = A_0 > 0$;
    \item поочерёдно проходить 5 квадрантов, нигде не обращаясь в ноль;
...
При $k_{\text{ПМ}} = 10$, $k_{\text{ИМ}} = 1$, $k_{\text{ДМ}} = 1$:
\[
A_0 = r k_{\text{ИМ}} (b i + c) = 0.7 \cdot 1 \cdot (4 \cdot 10 + 10000) = 0.7 \cdot 10040 = 7028 > 0
\]
...
\begin{tikzpicture}
    \begin{axis}[
        ...
        grid=both,
        axis lines=middle,
        title={Годограф Михайлова при $k_{\text{ПМ}}=10$, $k_{\text{ИМ}}=1$, $k_{\text{ДМ}}=1$},
        legend pos=north east,
        width=0.85\textwidth,
...
При $k_{\text{ПМ}} = 12$, $k_{\text{ИМ}} = 0.5$, $k_{\text{ДМ}} = 2.0$, $\Phi_0 = 1$:
\[
\phi_{\text{м}}(t) \approx 1 + 0.15 e^{-0.7t} \cos(10t - 0.3) + \text{быстрозатухающие составляющие}
\]

Переходный процесс затухает за время $t_{\text{п}} \approx 4.3$ с.
...
Условие: $A_0 = 0$ $\Rightarrow$ $k_{\text{ИМ}} = 0$.
\[
\phi_{\text{м}}(t) = C_0 + \sum_{k=1}^{4} C_k e^{\sigma_k t} \cos(\omega_k t + \varphi_k)
\]
...
\]
Возникают незатухающие колебания с частотой $\omega_0$.
...
\begin{tikzpicture}
    ...
    \end{axis}
\end{tikzpicture}
\caption{Область устойчивости и допустимых параметров на плоскости $(k_{\text{ПМ}}, k_{\text{ДМ}})$}
\end{figure}
...
\subsection*{Область $\{M\}$ на плоскости $(k_{\text{ПМ}}, k_{\text{ДМ}})$}
Условие:
\[
\max_{t \geq 0} |\ddot{\phi}_{\text{м}}(t; k_{\text{ПМ}}, k_{\text{ДМ}})| \leq 150
\]

\subsection*{Влияние параметров}
\begin{itemize}
    \item Увеличение $k_{\text{ПМ}}$ $\rightarrow$ рост момента
    \item Увеличение $k_{\text{ДМ}}$ $\rightarrow$ снижение момента
\end{itemize}

\subsection*{Совместная область $\{L\}$}
\[
\{L\} = \{ (k_{\text{ПМ}}, k_{\text{ДМ}}) : M_{\text{max}} \leq 150 \ \text{Н}\cdot\text{м} \ \text{и} \ u_{\text{max}} \leq 220 \ \text{В} \}
\]
...
\begin{tabular}{|c|c|c|}
\hline
$k_{\text{ПМ}}$ & $k_{\text{ДМ}}$ & $M_{\text{м,max}}$, Н$\cdot$м \\
\hline
5 & 0.5 & 80 \\
10 & 1 & 120 \\
15 & 1 & 180 \\
...
\[
u(t) = k_{\text{ПМ}} \bigl( \phi_{\text{м}}(t) - \phi^*(t) \bigr) + k_{\text{ИМ}} \int \bigl( \phi_{\text{м}}(t) - \phi^*(t) \bigr) dt + k_{\text{ДМ}} \bigl( \dot{\phi}_{\text{м}}(t) - \dot{\phi}^*(t) \bigr)
\]
...
При $t = 0$:
\[
u(0) = -k_{\text{ПМ}} \Phi_0 \quad\Rightarrow\quad k_{\text{ПМ}} \leq 220 \text{ при } \Phi_0 = 1
\]

\subsection*{Амплитуда колебаний}
При доминировании колебательной составляющей:
\[
U_{\text{ампл}} \approx A \sqrt{k_{\text{ПМ}}^2 + (k_{\text{ДМ}} \omega_0)^2}
\]

\subsection*{Область $\{U\}$}
\[
\{U\} = \left\{ (k_{\text{ПМ}}, k_{\text{ДМ}}) : \max_{t \geq 0} |u(t; k_{\text{ПМ}}, k_{\text{ДМ}})| \leq 220 \right\}
\]
...
\hline
$k_{\text{ПМ}}$ & $k_{\text{ДМ}}$ & $u_{\text{max}}$, В & Допустимо \\
\hline
10 & 1 & 25 & Да \\
50 & 1 & 60 & Да \\
100 & 1 & 120 & Да \\
...
\[
k_{\text{ПМ}} + A \sqrt{k_{\text{ПМ}}^2 + (k_{\text{ДМ}} \omega_0)^2} = 220
\]

Для вашей системы при $\Phi_0 = 1$ рад допустимые значения: $k_{\text{ПМ}} \lesssim 150$ при малых $k_{\text{ДМ}}$, и $k_{\text{ДМ}} \lesssim 5$ при $k_{\text{ПМ}} \approx 50$.
...
На плоскости $(k_{\text{ПМ}}, k_{\text{ДМ}})$ линии $t_{\text{п}} = \text{const}$ представляют собой замкнутые кривые, вложенные друг в друга. Минимальная длительность достигается в центре.

\subsection*{Минимальная длительность в области $\{L\}$}
\[
t_{\text{п,min}} \approx 3 \div 4\ \text{с}
\]
при $k_{\text{ПМ}} \approx 8 \div 12$, $k_{\text{ДМ}} \approx 1.5 \div 2.5$.

\subsection*{Максимальная длительность в области $\{L\}$}
При приближении к границе устойчивости ($k_{\text{ПМ}} \to 0$ или выход на колебательную границу):
\[
...
\begin{tabular}{|c|c|c|}
\hline
$t_{\text{п}}$, с & $k_{\text{ПМ}}$ & $k_{\text{ДМ}}$ \\
\hline
3 & 10 & 2.5 \\
4 & 8 & 1.5 \\
5 & 6 & 1 \\
...
\begin{tikzpicture}[scale=0.8]
    ...
    \draw[->] (0,0) -- (0,4) node[above] {$k_{\text{ДМ}}$};
...
На плоскости $(k_{\text{ПМ}}, k_{\text{ДМ}})$ линии представляют собой замкнутые кривые с минимумом в центре.
...
\begin{tabular}{|c|c|c|c|}
\hline
$k_{\text{ПМ}}$ & $k_{\text{ДМ}}$ & $\sigma_{\text{дом}}$ & $J_0$ (прибл.) \\
\hline
8 & 2 & -0.8 & 0.02 \\
10 & 1 & -0.5 & 0.09 \\
5 & 0.5 & -0.3 & 0.4 \\
...
Минимальное значение: $J_{0,\min} \approx 0.01 \div 0.03$ при $k_{\text{ПМ}} \approx 8 \div 12$, $k_{\text{ДМ}} \approx 2 \div 3$.
...
При $\rho \to \infty$ минимизация $J(\rho)$ требует $u(t) \to 0$, что достигается при $k_{\text{ПМ}} \to 0$, $k_{\text{ДМ}} \to 0$.

\subsection*{Асимптотическое поведение}
Оптимальные параметры и минимальное значение функционала:
\[
k_{\text{ПМ}}^* \sim \frac{1}{\sqrt{\rho}}, \quad k_{\text{ДМ}}^* \sim \frac{1}{\sqrt{\rho}}, \quad J_{\min}(\rho) \sim 2\sqrt{\rho}
\]

\subsection*{Линии равных уровней}
При фиксированном большом $\rho$ линии $J = \text{const}$ на плоскости $(k_{\text{ПМ}}, k_{\text{ДМ}})$ имеют вид:
\[
\frac{k_{\text{ПМ}}}{k_{\text{ДМ}}} = \text{const}
\]
...
\begin{tabular}{|c|c|c|c|}
\hline
$\rho$ & $k_{\text{ПМ}}^*$ & $k_{\text{ДМ}}^*$ & $J_{\min}(\rho)$ \\
\hline
$10^2$ & 0.1 & 0.01 & 20 \\
$10^4$ & 0.01 & 0.001 & 200 \\
...
\begin{tabular}{|c|c|c|c|c|}
\hline
$\rho$ & $k_{\text{ПМ}}^*$ & $k_{\text{ДМ}}^*$ & $M_{\text{max}}$ & $u_{\text{max}}$ \\
\hline
$10^{-4}$ & 15 & 2.5 & 180 & 200 \\
...
\[
k_{\text{ПМ}}^* = 12, \quad k_{\text{ИМ}}^* = 0.5, \quad k_{\text{ДМ}}^* = 2.0, \quad \rho^* = 5\cdot10^{-4}
\]
...
Оптимальная кривая — множество точек $(k_{\text{ПМ}}^*(\rho), k_{\text{ДМ}}^*(\rho))$, минимизирующих:
\[
...
\hline
$\rho$ & $k_{\text{ПМ}}^*$ & $k_{\text{ДМ}}^*$ & $J_{\min}$ & $M_{\text{max}}$ & $u_{\text{max}}$ \\
\hline
...
\[
\boxed{\rho^* = 5\cdot10^{-4},\quad k_{\text{ПМ}}^* = 12,\quad k_{\text{ДМ}}^* = 2.0,\quad k_{\text{ИМ}}^* = 0.5}
\]
...
\[
k_{\text{ПМ}} = 12, \quad k_{\text{ИМ}} = 0.5, \quad k_{\text{ДМ}} = 2.0, \quad \rho = 5 \cdot 10^{-4}
\]
...
\[
s_{1,2} \approx -0.7 \pm 10j, \quad \text{остальные корни имеют большую отрицательную вещественную часть}
\]
...
Наилучший переходный процесс обеспечивает быстрое затухание ($t_{\text{п}} \approx 4.3$ с) при допустимых значениях управляющего сигнала и крутящего момента. Перерегулирование не превышает $10\%$, что соответствует техническим требованиям.
        ...
        \section*{Выводы}
В ходе выполнения курсовой работы был проведён полный цикл анализа и синтеза аналогового ПИД-регулятора для модуля поворота промышленного робота. Получены следующие основные результаты.
...
С использованием критериев Стодолы и Рауса-Гурвица определена область устойчивости замкнутой системы на плоскости параметров регулятора $(k_{\text{ПМ}}, k_{\text{ДМ}})$ при фиксированном $k_{\text{ИМ}}$:
\begin{itemize}
    \item Необходимое условие Стодолы выполняется при $k_{\text{ПМ}} > 0$, $k_{\text{ИМ}} > 0$, $k_{\text{ДМ}} > 0$;
    \item Критерий Рауса-Гурвица задаёт границы устойчивости.
\end{itemize}
...
На плоскости $(k_{\text{ПМ}}, k_{\text{ДМ}})$ построены:
\begin{itemize}
...
\begin{itemize}
    ...
    \item На апериодической границе ($k_{\text{ИМ}} = 0$) возникает нулевой корень, система становится астатической;
    \item На колебательной границе появляются незатухающие колебания.
\end{itemize}
...
\[
J_{\text{opt}}(\rho) = \min_{k_{\text{ПМ}}, k_{\text{ИМ}}, k_{\text{ДМ}}} \int_{0}^{\infty} \left( \psi_{\text{м}}^2 + \rho u^2 \right) dt.
\]
Исследованы случаи:
\begin{itemize}
    \item $\rho = 0$ — минимизация только ошибки;
    \item $\rho \to \infty$ — минимизация управления;
...
\[
\boxed{k_{\text{ПМ}} = 12,\quad k_{\text{ИМ}} = 0.5,\quad k_{\text{ДМ}} = 2.0,\quad \rho^* = 5\cdot10^{-4}}.
\]
...
Разработанный аналоговый ПИД-регулятор с параметрами $k_{\text{ПМ}} = 12$, $k_{\text{ИМ}} = 0.5$, $k_{\text{ДМ}} = 2.0$ обеспечивает:
\begin{itemize}
